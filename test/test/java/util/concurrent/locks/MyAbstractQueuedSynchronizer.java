...
public abstract class MyAbstractQueuedSynchronizer extends AbstractOwnableSynchronizer implements java.io.Serializable {
    ...
    public void printCallerInfo(String what, Object... forPrint) {
        // 不一定是真实输出的顺序和时间，但时间一般来说不会有大的出入
        ...
        sb.append('[').append(t1).append("]Thread[").append(name).append("] do[").append(what);
        sb.append("] last stack trace is[").append(ste.getClassName()).append('#').append(ste.getMethodName());
        sb.append(']');
    ...
    private void setHead(Node node) {
        printCallerInfo("try set head", node);
        head = node;
        node.thread = null;
        node.prev = null;
        printCallerInfo("set head done");
    }
    ...
    private void doReleaseShared() {
        ...
        for (; ; ) {
            // 尝试唤醒等待队列首节点
            Node h = head;
            printCallerInfo("doReleaseShared before unpark head", h);
    ...
    private final boolean compareAndSetHead(Node update) {
        printCallerInfo("try compareAndSetHead", update);
        boolean r = unsafe.compareAndSwapObject(this, headOffset, null, update);
        printCallerInfo("try compareAndSetHead done", r);
        return r;
    }
