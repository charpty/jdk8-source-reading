 ...
 *
 * 该类提供了在线程本地存储变量的能力。这些存储的变量并不是每个线程保存一个副本的形式。
 * 该类的实例应该总是以静态变量形式出现，用于存储不同线程间的变量值。
 *
 *
 ...
 *
 * 每一个线程都隐式持有本地线程变量引用，知道线程死亡或者ThreadLocal实例不可访问。
 * 当一个线程被销毁，ThreadLocal所持有的变量也会被回收。
 *
...
public class ThreadLocal<T> {
	 ...
	 * less common cases.
	 *
	 * ThreadLocal使用实例自身作为Key。类似于HashMap的结构，但是冲突时没有链表。
	 * 为了消除Hash冲突采用更好的Hash分配策略
	 */
	 ...
	 *
	 * 供使用者实现初始化值的方式，在这里初始化会免去使用者各种锁处理带来的麻烦。
	 * 只要还没有set过值，第一次调用get时会调用该方法。
	 * 只要不调用remove方法，初始化方法只会被调用一次。
	 *
	...
	static class ThreadLocalMap {
		 ...
		 *
		 * @param i
		 * 		a position known NOT to hold a stale entry. The
		 * 		scan starts at the element after i.
		 * @param n
		 * 		scan control: {@code log2(n)} cells are scanned,
