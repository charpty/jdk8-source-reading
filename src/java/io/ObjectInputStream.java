...
public class ObjectInputStream extends InputStream implements ObjectInput, ObjectStreamConstants {
    ...
    static {
        ...
        // charpty: comment because compile error
     ...
     *         if the {@linkplain #getInternalObjectInputFilter() current filter}
     ...
     * @param arrayLength
     *         the array length requested; use {@code -1} if not creating an array
     *
     * @throws InvalidClassException
     *         if it rejected by the filter or
    ...
    private void filterCheck(Class<?> clazz, int arrayLength) throws InvalidClassException {
        if (serialFilter != null) {
            ...
            try {
                status = serialFilter.checkInput(new FilterValues(clazz, arrayLength, totalObjectRefs, depth, bin.getBytesRead()));
            ...
            if (status == null || status == ObjectInputFilter.Status.REJECTED) {
                ...
                if (Logging.infoLogger != null) {
                    Logging.infoLogger
                            .info("ObjectInputFilter {0}: {1}, array length: {2}, nRefs: {3}, depth: {4}, bytes: {5}, ex: {6}", status, clazz, arrayLength,
                                    totalObjectRefs, depth, bin.getBytesRead(), Objects.toString(ex, "n/a"));
                }
                ...
                if (Logging.traceLogger != null) {
                    Logging.traceLogger
                            .finer("ObjectInputFilter {0}: {1}, array length: {2}, nRefs: {3}, depth: {4}, bytes: {5}, ex: {6}", status, clazz, arrayLength,
                                    totalObjectRefs, depth, bin.getBytesRead(), Objects.toString(ex, "n/a"));
                }
            }
        }
