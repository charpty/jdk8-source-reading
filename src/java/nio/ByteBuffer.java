 ...
 *
 * 在指定位置读取或放入一个char，char和byte的字节转换将在内部完成
 *
 ...
 *
 * ByteBuff可以使用allocate()重新分配一个，也可以使用wrap()用一个既有的数组初始化为ByteBuffer
 *
 *
 * <a name="direct"></a>
 ...
 *
 * ByteBuffer可以是堆内或在堆外进行分配
 * 如果直接在堆外（直接在操作系统层面）分配，JVM能更好的利用本地内存完成I/O操作
 * 使用本地内存，JVM省去了进行I/O操作时，在虚拟机和本地操作系统之间大量的内存拷贝工作
 *
 ...
 *
 * 分配堆外内存可以通过allocateDirect()函数来实现，堆外内存分配与回收需要更高的开销
 * 堆外内存超出了GC的回收的范畴，所以有可能放任他们造成系统内存泄漏
 * 所以建议仅仅在需要大块内存并大量需要使用操作系统I/O时才申请堆外内存
 * 总队来说，仅当使用堆外内存能带来可观的性能提升时才使用它们
 *
 ...
 *
 * 堆外内存也可以通过FileChannel#map将一个文件局部的部分内存直接映射为Java句柄
 * 这种映射操作同样支持通过JNI来完成
 * 映射的内存也存在访问权限属性，访问不可访问的内存区域将抛出指定异常
 *
 * <p> Whether a byte buffer is direct or non-direct may be determined by
 * invoking its {@link #isDirect isDirect} method.  This method is provided so
 * that explicit buffer management can be done in performance-critical code.
 *
 * ByteBuffer提供了函数isDirect()用于判断是否为堆外内存，有利于在高性能要求的编程中提供自主管理
 *
 * <a name="bin"></a>
