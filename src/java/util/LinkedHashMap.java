 ...
 * the invocation.)
 *
 * 集合链表实现排序的哈希表实现。该类实现采用了一个双向链表连接了Map中所有的entry。
 * 通过链表实现顺序，一般来说是通过元素插入的顺序进行排序。
 * 但是当put一个已有Key时，实际上只是修改了所在entry当value值，此时需要在afterNodeAccess回调函数中进行排序修改，
 * 因为这种情况相当于是重新插入了。
 *
 ...
 * </pre>
 *
 * 该类为HashMap提供了一个简明扼要的排序版本实现，而且无需像TreeMap那样大费周章。
 * 该类构造器允许在保留原Map既有元素顺序的情况下构造一个LinkedHashMap。
 *
 ...
 *
 * 当调用者需要构建一个新的具有排序功能的Map且要保留原Map元素顺序时，这个构造函数是非常有用的。
 *
