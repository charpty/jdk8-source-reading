 ...
 *
 * 一个并发且可伸缩的ConcurrentNavigableMap实现。
 * Map中的元素都是有序的，排序是根据键的自然顺序（实现了Comparable接口）或者构造Map时指定的Comparator来实现的。
 *
 ...
 * multiple threads.
 *
 * 该类的实现是跳跃表的一种变体，可以让Map的增、删、改等操作的时间复杂度为O(log(n))。
 * 这些对Map的访问和修改操作都是线程安全的。
 *
 * <p>Iterators and spliterators are
 * <a href="package-summary.html#Weakly"><i>weakly consistent</i></a>.
 *
 * 对Map中的元素进行迭代或者将Map切分为子Map的操作只能保证弱一致性。
 *
 ...
 *
 * 值得注意的是，本实现和其他集合实现不同，获取Map的大小是需要遍历整个Map的（大多数集合都是用一个size变量直接存储集合大小）。
 * 由于该Map实现的异步特性，获取该Map的元素个数需要遍历整个Map，由于Map有可能在遍历时被修改，所以最终结果数量可能是不准确的。
 * 另外，许多批量型操作，如putAll、toArray等都无法保证绝对的准确性。
 * 比如获取迭代器A的操作和将一批元素B放进Map的操作如果并发进行，那么无法保证A能看到全部B中的元素，或许只能看到部分。
 *
 ...
 * elements.
 *
 * 该类实现了标准Map和标准Iterator的所有操作。和许多并发容器一样，该类不允许空的键和值。
 * 这是由于部分函数或属性为null有可能是由于并发时数据竞争导致的，如果容器包含null值，那么就无法区分到底是数据竞争还是真的值为null。
 *
...
public class ConcurrentSkipListMap<K, V> extends AbstractMap<K, V> implements ConcurrentNavigableMap<K, V>, Cloneable, Serializable {
     ...
     * +-+  +-+  +-+  +-+  +-+  +-+  +-+  +-+  +-+  +-+  +-+  +-+
     *
     * 这个类的实现使用的是类似树结果的二维跳跃表，用于索引的层和存储数据的层是独立分开的。
     * 之所以不使用传统的数组结构有以下两点原因：
     * 1）基于数组的实现更加的复杂且开销更大
     * 2）将索引层分离因此我们可以在该层上使用更加轻量的算法来应对大量遍历操作
     *
     ...
     * nested classes.)
     *
     * 全局比较器，用于对键进行比较，未设置的情况下则使用键的自然顺序（Comparable接口）。
     */
    final Comparator<? super K> comparator;

    /** Lazily initialized key set */
    /** 该Map的keySet，仅在第一次获取时才生成 */
    private transient KeySet<K> keySet;
    /** Lazily initialized entry set */
    /** 该Map的entry集合，仅在第一次获取时才生成 */
    private transient EntrySet<K, V> entrySet;
    /** Lazily initialized values collection */
    /** 该Map的值集合，仅在第一次获取时才生成 */
     ...
     *
     * 跳跃表节点，按Key顺序逐一相连，跳跃表中包含实际数据的节点也存在一些标记节点（目前都代表删除标记）。
     * 跳跃表的头节点是一个虚假的节点。
     * 存储值的属性使用的是Object类型而不是V，这是因为虽然数据是V类型，但是标记节点和头节点则不是。
     */
    static final class Node<K, V> {
         ...
         *
         * 检查当前节点是否是一个标记节点。
         * 实际上该方法目前没有使用，主要是由于调用者其实已经获取到Node的V值，此时只要直接判断下v==node即可
         *
        ...
        void helpDelete(Node<K, V> b, Node<K, V> f) {
             ...
             *
             * 调用这个函数则说明当前节点已经被删除了（其它线程删除的）。
             * 此时要做的就是将前驱节点b的后继节点设置为f。
             *
             */
            // 如果f是当前节点的后继节点，当前节点又是b的后继节点，则说明当前的节点的前后的指针关系还没有发生变化
            // 那么此时就由当前线程来"推一把"，完成删除节点前后指针的改变（即真正的删除节点）
            if (f == next && this == b.next) {
                if (f == null || f.value != f) // not already marked
                {
                    // 这里使用标记节点代表删除节点
                    // 如果f不是一个标记节点则将当前节点的后继节点设置为一个标记节点，意味着当前节点接下来可以安全的移除了
                    casNext(f, new Node<K, V>(f));
                } else {
                    // 如果f是一个标记节点，那么意味着可以安全移除该节点了，那么就将b的后继节点设置f的下一个节点
                    // 设置成功之后由于没有任何引用指向当前节点，那么相当于当前节点被彻底从列表中移除了
     ...
     *
     * 找出比指定Key小的上限值（比Key小的值里最大的），并返回其在底层level上的node引用。
     * 这个函数的执行过程中也顺便删掉了发现的已经标记删除的节点（helpDelete函数）。
     *
     * @param key
     *         the key
     *
    ...
    private Node<K, V> findPredecessor(Object key, Comparator<? super K> cmp) {
        ...
        for (; ; ) {
            ...
            for (Index<K, V> q = head, r = q.right, d; ; ) {
                if (r != null) {
                    // 如果当层层内（右边）还有节点，则先尽量从高层往右找，找到第一个大于当前Key的节点
                    Node<K, V> n = r.node;
                    K k = n.key;
                    // 当前节点的值为null则说明当前节点已经被删除了
                    if (n.value == null) {
                        // 尝试将已删除节点的前驱节点q的后继节点设置为r的后继节点（这样r就没有任何引用了）
                        if (!q.unlink(r)) {
                            // 设置失败则重新开始内层遍历
                            // TODO 为啥要从头开始呢？
                            break;           // restart
                        }
                        // 成功删除当前节点之后r就变成r的下一个节点了，也就是目前q的下一个节点，此时仅需重读一下r
                        r = q.right;         // reread r
                        continue;
                    }
                    // 如果当前r的key还小于指定key则继续往右遍历，直到碰到大于指定key的
                    if (cpr(cmp, key, k) > 0) {
                        q = r;
                        r = r.right;
                        continue;
                ...
                // 此时说明key在(q.node,r.node)之间，现在往下走一层，进一步细化值区间
                if ((d = q.down) == null) {
                    // 如果已经没法往下走，也就是没法细化值区间了，那么目前值就是小于指定key值且最接近的值了。
                    return q.node;
                }
                // 往下一层
                q = d;
     ...
     *         the key
     * @param value
     *         if non-null, the value that must be
     *         associated with key
     *
    ...
    private void tryReduceLevel() {
        ...
        HeadIndex<K, V> d;
        HeadIndex<K, V> e;
        if (h.level > 3 && (d = (HeadIndex<K, V>) h.down) != null && (e = (HeadIndex<K, V>) d.down) != null && e.right == null && d.right == null
        ...
        public Spliterator<K> trySplit() {
            ...
            if ((e = current) != null && (ek = e.key) != null) {
                for (Index<K, V> q = row; q != null; q = row = q.down) {
                    ...
                    Node<K, V> b, n;
                    K sk;
                    if ((s = q.right) != null && (b = s.node) != null && (n = b.next) != null && n.value != null && (sk = n.key) != null && cpr(cmp, sk, ek) > 0
