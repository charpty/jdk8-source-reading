...
public class ReentrantLock implements Lock, java.io.Serializable {
	 ...
	 *
	 * 这是重入锁使用的基础同步器，这是个抽象类，具体的的实现有公平同步器和非公平同步器。
	 * 使用父类AQS的state变量存储锁定线程多少次锁住了该锁（重入锁基本的语义就是统一线程锁了多少次就解锁多少次）
	 */
	abstract static class Sync extends AbstractQueuedSynchronizer {
		 ...
		 *
		 * 非公平的资源竞争释放操作。tryAcquire这个方法留个子类自行实现。
		 */
		final boolean nonfairTryAcquire(int acquires) {
			final Thread current = Thread.currentThread();
			// 获取状态，此时了此操作之后，不论是否锁住，线程都能看到同样使用该变量判断锁的线程对其它变量对写入值
			int c = getState();
			if (c == 0) {
				// 如果当前锁未被持有则尝试使用CAS方式设置锁状态，成功之后则将当前锁的持有者设置为当前线程
				if (compareAndSetState(0, acquires)) {
					setExclusiveOwnerThread(current);
					return true;
				}
			} else if (current == getExclusiveOwnerThread()) {
				// 虽然锁被锁住了，但是锁持有者就是当前线程，那么允许重入，并将锁住的次数加1
				...
				// 此处不需要同步操作，因为锁的持有者只有一个线程
				setState(nextc);
				return true;
			}
			// 返回告诉AQS当前没有资源可用，即锁已被其它线程锁住
			return false;
		}

		protected final boolean tryRelease(int releases) {
			// 释放锁，执行该操作之后所有设置state之前的写入值都对当前线程可见
			...
			boolean free = false;
			if (c == 0) {
				// 如果持锁者持锁次数已经降为0了则说明释放了锁
				free = true;
				setExclusiveOwnerThread(null);
			}
			// 程序运行到这里必然是拥有锁到线程执行的，因为是独占模式，所以只有一个线程，无需考虑竞争问题
			setState(c);
			return free;
		}
