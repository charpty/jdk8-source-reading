...
final class DualPivotQuicksort {
	 ...
	 *
	 * byte数组长度超过该域值时则使用计数排序，数组较小时则使用插入排序
	 */
	...
	static void sort(byte[] a, int left, int right) {
		...
		if (right - left > COUNTING_SORT_THRESHOLD_FOR_BYTE) {
			...
			// 首先计算出各个值出现的次数，数组的下标也就是byte值离Byte.MIN_VALUE的距离
			...
			for (int i = NUM_BYTE_VALUES, k = right + 1; k > left; ) {
				...
				// 下标+Byte.MIN_VALUE也就是byte值，出现了多少次也就是排到多少位
				byte value = (byte) (i + Byte.MIN_VALUE);
				// 值出现了s次，就把该值按顺序插入到结果数据数组（也就是入参数组）中s次即可
				// 遍历从尾部开始，所以也从尾部开始插入，一个值的顺序的下标也就是前面的值出现次数的总和
				int s = count[i];
